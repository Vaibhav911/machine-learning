...
# Read dataset 1 and 2, and randomly split it into training
...
data_2 = pd.read_csv("./../data/a1_data/a1_d2.csv", names=["x", "y", "label"])
mask_2 = np.random.rand(len(data_2)) < 0.8
data_2_train = data_2[mask_2]
data_2_test = data_2[~mask_2]
...
data_2_train = data_2_train.reset_index()
data_2_test = data_2_test.reset_index()
...
def main():
    ...
    wts = pd.DataFrame(wts, index=['x', 'y'])
    (c1_pnts, c2_pnts) = transform(data_2, wts)
